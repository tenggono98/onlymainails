<div>
    {{-- Nothing in the world is as soft and yielding as water. --}}


    {{-- HERO ZONE --}}
    <section id="hero">
        <div class="lg:grid lg:grid-cols-7 lg:grid-flow-col">
            <div class="lg:col-span-3">
                <img src="{{ asset('img/IMG_3915.png') }}" class="object-cover w-full rounded-lg lg:h-full h-72 lg:w-auto" alt="">
            </div>
            <div class="p-5 text-justify lg:col-span-4 lg:px-40 lg:py-14">

                <h1 class="text-4xl leading-loose tracking-wide text-center uppercase lg:text-left ">Only Mai Nails</h1>

                <p class="mb-5 leading-loose">
                    Lorem ipsum dolor sit amet consectetur adipisicing elit. Eum provident eius dolores accusantium totam quam odit? Commodi temporibus dolorem mollitia.
                </p>

                <p class="mb-5 leading-loose">
                    Lorem ipsum dolor sit amet consectetur adipisicing elit. Delectus, hic.
                </p>

                <p class="mb-5 leading-loose">
                    Lorem ipsum dolor sit amet consectetur, adipisicing elit. Consequatur maiores sunt nemo voluptatibus amet voluptas officia, ipsum vero nulla facilis in dignissimos laboriosam necessitatibus deserunt totam nihil odio. Veritatis, illo.
                </p>


            </div>
        </div>

    </section>
    {{-- HERO ZONE --}}



    {{-- BANNER ZONE --}}
    <section id="banner" class="py-10 ">

        <div class="py-5 ">
            <p class="text-4xl text-center animate-fade animate-once animate-delay-100">WELCOME TO <span class="leading-loose typed-out">ONLYMAINAILS</span> </p>
        </div>


    </section>
    {{-- BANNER ZONE --}}


     {{-- About Us ZONE --}}

     <section id="about-us">
        <div class="lg:grid lg:grid-cols-7 lg:grid-flow-col">

            <div class="p-5 text-justify lg:col-span-4 lg:px-40 lg:py-14">

                <h1 class="text-4xl leading-loose tracking-wide text-center uppercase lg:text-left ">About Us</h1>

                <p class="mb-5 leading-loose">
                    Lorem ipsum dolor sit amet consectetur adipisicing elit. Eum provident eius dolores accusantium totam quam odit? Commodi temporibus dolorem mollitia.
                </p>

                <p class="mb-5 leading-loose">
                    Lorem ipsum dolor sit amet consectetur adipisicing elit. Delectus, hic.
                </p>

                <p class="mb-5 leading-loose">
                    Lorem ipsum dolor sit amet consectetur, adipisicing elit. Consequatur maiores sunt nemo voluptatibus amet voluptas officia, ipsum vero nulla facilis in dignissimos laboriosam necessitatibus deserunt totam nihil odio. Veritatis, illo.
                </p>


            </div>

            <div class="lg:col-span-3">
                <img src="{{ asset('img/IMG_3916.png') }}" class="object-cover w-full rounded-lg lg:h-full h-72 lg:w-auto" alt="">
            </div>
        </div>

    </section>







    {{-- About Us ZONE --}}



     {{-- BANNER ZONE --}}
     <section id="banner" class="py-10 ">

        <div class="py-5 ">
            <p class="text-4xl text-center uppercase">WHY CHOOSE US? </p>
        </div>


    </section>
    {{-- BANNER ZONE --}}


    {{-- WHY US ZONE --}}

    <section id="why-us">
        <div class="lg:grid lg:grid-cols-7 lg:grid-flow-col">
            <div class="lg:col-span-3">
                <img src="{{ asset('img/IMG_3917.png') }}" class="object-cover w-full rounded-lg lg:h-full h-72 lg:w-auto" alt="">
            </div>

            <div class="p-5 text-justify lg:col-span-4 lg:px-40 lg:py-14">

                <h1 class="text-4xl leading-loose tracking-wide text-center uppercase lg:text-left ">Why Us</h1>

                <p class="mb-5 leading-loose">
                    Lorem ipsum dolor sit amet consectetur adipisicing elit. Eum provident eius dolores accusantium totam quam odit? Commodi temporibus dolorem mollitia.
                </p>

                <p class="mb-5 leading-loose">
                    Lorem ipsum dolor sit amet consectetur adipisicing elit. Delectus, hic.
                </p>

                <p class="mb-5 leading-loose">
                    Lorem ipsum dolor sit amet consectetur, adipisicing elit. Consequatur maiores sunt nemo voluptatibus amet voluptas officia, ipsum vero nulla facilis in dignissimos laboriosam necessitatibus deserunt totam nihil odio. Veritatis, illo.
                </p>


            </div>


        </div>

    </section>

    {{-- WHY US ZONE --}}

     {{-- BANNER ZONE --}}
     <section id="banner" class="py-10 ">

        <div class="py-5 ">
            <p class="text-4xl text-center uppercase">Services </p>
        </div>


    </section>
    {{-- BANNER ZONE --}}


    {{-- SERVICES ZONE --}}

    <section id="services" class="">

        <div class="">


            <div class="grid gap-4 lg:grid-cols-4">
                {{-- Card Gel-X --}}
                <div class="p-4 border rounded-lg">
                    <h1 class="text-2xl text-center">Gel-X</h1>
                    {{-- Item --}}
                    <div class="">
                        <div class="flex justify-between">
                            <div class="">
                                <p>Short</p>
                            </div>
                            <div class="">
                                <p>$75</p>
                            </div>

                        </div>
                    </div>
                    {{-- Item --}}
                     {{-- Item --}}
                     <div class="">
                        <div class="flex justify-between">
                            <div class="">
                                <p>Medium</p>
                            </div>
                            <div class="">
                                <p>$80</p>
                            </div>

                        </div>
                    </div>
                    {{-- Item --}}
                     {{-- Item --}}
                     <div class="">
                        <div class="flex justify-between">
                            <div class="">
                                <p>Fills</p>
                            </div>
                            <div class="">
                                <p>$85</p>
                            </div>

                        </div>
                    </div>
                    {{-- Item --}}
                    <p class="text-xs font-thin">Only permitted once for Gel-X</p>
                </div>
                {{-- Card --}}

                 {{-- Card BGO --}}
                 <div class="p-4 border rounded-lg">
                    <h1 class="text-2xl text-center">Builder Gel Overlay</h1>
                    {{-- Item --}}
                    <div class="">
                        <div class="flex justify-between">
                            <div class="">
                                <p>Short</p>
                            </div>
                            <div class="">
                                <p>$55</p>
                            </div>

                        </div>
                    </div>
                    {{-- Item --}}
                     {{-- Item --}}
                     <div class="">
                        <div class="flex justify-between">
                            <div class="">
                                <p>Medium</p>
                            </div>
                            <div class="">
                                <p>$60</p>
                            </div>

                        </div>
                    </div>
                    {{-- Item --}}
                     {{-- Item --}}
                     <div class="">
                        <div class="flex justify-between">
                            <div class="">
                                <p>Long</p>
                            </div>
                            <div class="">
                                <p>$65</p>
                            </div>

                        </div>
                    </div>
                    {{-- Item --}}
                </div>
                {{-- Card --}}

                 {{-- Card Removals --}}
                 <div class="p-4 border rounded-lg">
                    <h1 class="text-2xl text-center">Removals</h1>
                    {{-- Item --}}
                    <div class="">
                        <div class="flex justify-between">
                            <div class="">
                                <p>Removal</p>
                            </div>
                            <div class="">
                                <p>$20</p>
                            </div>

                        </div>
                    </div>
                    {{-- Item --}}
                     {{-- Item --}}
                     <div class="">
                        <div class="flex justify-between">
                            <div class="">
                                <p>Removal w/ Set</p>
                            </div>
                            <div class="">
                                <p>$10</p>
                            </div>

                        </div>
                    </div>
                    {{-- Item --}}
                     {{-- Item --}}
                     <div class="">
                        <div class="flex justify-between">
                            <div class="">
                                <p>Foreign Removal</p>
                            </div>
                            <div class="">
                                <p>$30</p>
                            </div>

                        </div>
                    </div>
                    {{-- Item --}}
                    {{-- Item --}}
                    <div class="">
                        <div class="flex justify-between">
                            <div class="">
                                <p>Foreign Removal w/ Set</p>
                            </div>
                            <div class="">
                                <p>$30</p>
                            </div>

                        </div>
                    </div>
                    {{-- Item --}}
                </div>
                {{-- Card --}}

                {{-- Card Other Services --}}
                <div class="p-4 border rounded-lg">
                    <h1 class="text-2xl text-center">Other Services</h1>
                    {{-- Item --}}
                    <div class="">
                        <div class="flex justify-between">
                            <div class="">
                                <p>Basic Gel Manicure</p>
                            </div>
                            <div class="">
                                <p>$50</p>
                            </div>

                        </div>
                    </div>
                    {{-- Item --}}
                     {{-- Item --}}
                     <div class="">
                        <div class="flex justify-between">
                            <div class="">
                                <p>French Manicure</p>
                            </div>
                            <div class="">
                                <p>$40</p>
                            </div>

                        </div>
                    </div>
                    {{-- Item --}}
                     {{-- Item --}}
                     <div class="">
                        <div class="flex justify-between">
                            <div class="">
                                <p>Nail Designs</p>
                            </div>
                            <div class="">
                                <p>+$15</p>
                            </div>

                        </div>
                    </div>
                    {{-- Item --}}

                <p class="text-xs font-thin">DM for price quote!</p>

                </div>
                {{-- Card --}}


            </div>
        </div>

    </section>

    {{-- SERVICES ZONE --}}


    {{-- BANNER ZONE --}}
    <section id="banner" class="py-10 ">

        <div class="py-5 ">
            <p class="text-4xl text-center uppercase">WHAT OUR BEAUTIFUL & HAPPY CLIENTS SAY </p>
        </div>


    </section>
    {{-- BANNER ZONE --}}

    {{-- Review ZONE --}}

    <section id="review">


        <div class="grid gap-4 lg:grid-cols-3">

            {{-- Card  --}}
            <div class="p-4 border rounded-lg">

            <article>
                <div class="flex items-center mb-4">
                    <img class="w-10 h-10 rounded-full me-4" src="/docs/images/people/profile-picture-5.jpg" alt="">
                    <div class="font-medium dark:text-white">
                        <p>Jese Leos <time datetime="2014-08-16 19:00" class="block text-sm text-gray-500 dark:text-gray-400">Joined on August 2014</time></p>
                    </div>
                </div>
                <div class="flex items-center mb-1 space-x-1 rtl:space-x-reverse">
                    <svg class="w-4 h-4 text-yellow-300" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 22 20">
                        <path d="M20.924 7.625a1.523 1.523 0 0 0-1.238-1.044l-5.051-.734-2.259-4.577a1.534 1.534 0 0 0-2.752 0L7.365 5.847l-5.051.734A1.535 1.535 0 0 0 1.463 9.2l3.656 3.563-.863 5.031a1.532 1.532 0 0 0 2.226 1.616L11 17.033l4.518 2.375a1.534 1.534 0 0 0 2.226-1.617l-.863-5.03L20.537 9.2a1.523 1.523 0 0 0 .387-1.575Z"/>
                    </svg>
                    <svg class="w-4 h-4 text-yellow-300" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 22 20">
                        <path d="M20.924 7.625a1.523 1.523 0 0 0-1.238-1.044l-5.051-.734-2.259-4.577a1.534 1.534 0 0 0-2.752 0L7.365 5.847l-5.051.734A1.535 1.535 0 0 0 1.463 9.2l3.656 3.563-.863 5.031a1.532 1.532 0 0 0 2.226 1.616L11 17.033l4.518 2.375a1.534 1.534 0 0 0 2.226-1.617l-.863-5.03L20.537 9.2a1.523 1.523 0 0 0 .387-1.575Z"/>
                    </svg>
                    <svg class="w-4 h-4 text-yellow-300" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 22 20">
                        <path d="M20.924 7.625a1.523 1.523 0 0 0-1.238-1.044l-5.051-.734-2.259-4.577a1.534 1.534 0 0 0-2.752 0L7.365 5.847l-5.051.734A1.535 1.535 0 0 0 1.463 9.2l3.656 3.563-.863 5.031a1.532 1.532 0 0 0 2.226 1.616L11 17.033l4.518 2.375a1.534 1.534 0 0 0 2.226-1.617l-.863-5.03L20.537 9.2a1.523 1.523 0 0 0 .387-1.575Z"/>
                    </svg>
                    <svg class="w-4 h-4 text-yellow-300" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 22 20">
                        <path d="M20.924 7.625a1.523 1.523 0 0 0-1.238-1.044l-5.051-.734-2.259-4.577a1.534 1.534 0 0 0-2.752 0L7.365 5.847l-5.051.734A1.535 1.535 0 0 0 1.463 9.2l3.656 3.563-.863 5.031a1.532 1.532 0 0 0 2.226 1.616L11 17.033l4.518 2.375a1.534 1.534 0 0 0 2.226-1.617l-.863-5.03L20.537 9.2a1.523 1.523 0 0 0 .387-1.575Z"/>
                    </svg>
                    <svg class="w-4 h-4 text-gray-300 dark:text-gray-500" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 22 20">
                        <path d="M20.924 7.625a1.523 1.523 0 0 0-1.238-1.044l-5.051-.734-2.259-4.577a1.534 1.534 0 0 0-2.752 0L7.365 5.847l-5.051.734A1.535 1.535 0 0 0 1.463 9.2l3.656 3.563-.863 5.031a1.532 1.532 0 0 0 2.226 1.616L11 17.033l4.518 2.375a1.534 1.534 0 0 0 2.226-1.617l-.863-5.03L20.537 9.2a1.523 1.523 0 0 0 .387-1.575Z"/>
                    </svg>
                    <h3 class="text-sm font-semibold text-gray-900 ms-2 dark:text-white">Thinking to buy another one!</h3>
                </div>
                <footer class="mb-5 text-sm text-gray-500 dark:text-gray-400"><p>Reviewed in the United Kingdom on <time datetime="2017-03-03 19:00">March 3, 2017</time></p></footer>
                <p class="mb-2 text-gray-500 dark:text-gray-400">This is my third Invicta Pro Diver. They are just fantastic value for money. This one arrived yesterday and the first thing I did was set the time, popped on an identical strap from another Invicta and went in the shower with it to test the waterproofing.... No problems.</p>
                <p class="mb-3 text-gray-500 dark:text-gray-400">It is obviously not the same build quality as those very expensive watches. But that is like comparing a Citroën to a Ferrari. This watch was well under £100! An absolute bargain.</p>
                <a href="#" class="block mb-5 text-sm font-medium text-blue-600 hover:underline dark:text-blue-500">Read more</a>
                <aside>
                    <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">19 people found this helpful</p>
                    <div class="flex items-center mt-3">
                        <a href="#" class="px-2 py-1.5 text-xs font-medium text-gray-900 focus:outline-none bg-white rounded-lg border border-gray-200 hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-4 focus:ring-gray-100 dark:focus:ring-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:text-white dark:hover:bg-gray-700">Helpful</a>
                        <a href="#" class="text-sm font-medium text-blue-600 border-gray-200 ps-4 hover:underline dark:text-blue-500 ms-4 border-s md:mb-0 dark:border-gray-600">Report abuse</a>
                    </div>
                </aside>
            </article>

            </div>
            {{-- Card --}}

             {{-- Card  --}}
             <div class="p-4 border rounded-lg">

                <article>
                    <div class="flex items-center mb-4">
                        <img class="w-10 h-10 rounded-full me-4" src="/docs/images/people/profile-picture-5.jpg" alt="">
                        <div class="font-medium dark:text-white">
                            <p>Jese Leos <time datetime="2014-08-16 19:00" class="block text-sm text-gray-500 dark:text-gray-400">Joined on August 2014</time></p>
                        </div>
                    </div>
                    <div class="flex items-center mb-1 space-x-1 rtl:space-x-reverse">
                        <svg class="w-4 h-4 text-yellow-300" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 22 20">
                            <path d="M20.924 7.625a1.523 1.523 0 0 0-1.238-1.044l-5.051-.734-2.259-4.577a1.534 1.534 0 0 0-2.752 0L7.365 5.847l-5.051.734A1.535 1.535 0 0 0 1.463 9.2l3.656 3.563-.863 5.031a1.532 1.532 0 0 0 2.226 1.616L11 17.033l4.518 2.375a1.534 1.534 0 0 0 2.226-1.617l-.863-5.03L20.537 9.2a1.523 1.523 0 0 0 .387-1.575Z"/>
                        </svg>
                        <svg class="w-4 h-4 text-yellow-300" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 22 20">
                            <path d="M20.924 7.625a1.523 1.523 0 0 0-1.238-1.044l-5.051-.734-2.259-4.577a1.534 1.534 0 0 0-2.752 0L7.365 5.847l-5.051.734A1.535 1.535 0 0 0 1.463 9.2l3.656 3.563-.863 5.031a1.532 1.532 0 0 0 2.226 1.616L11 17.033l4.518 2.375a1.534 1.534 0 0 0 2.226-1.617l-.863-5.03L20.537 9.2a1.523 1.523 0 0 0 .387-1.575Z"/>
                        </svg>
                        <svg class="w-4 h-4 text-yellow-300" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 22 20">
                            <path d="M20.924 7.625a1.523 1.523 0 0 0-1.238-1.044l-5.051-.734-2.259-4.577a1.534 1.534 0 0 0-2.752 0L7.365 5.847l-5.051.734A1.535 1.535 0 0 0 1.463 9.2l3.656 3.563-.863 5.031a1.532 1.532 0 0 0 2.226 1.616L11 17.033l4.518 2.375a1.534 1.534 0 0 0 2.226-1.617l-.863-5.03L20.537 9.2a1.523 1.523 0 0 0 .387-1.575Z"/>
                        </svg>
                        <svg class="w-4 h-4 text-yellow-300" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 22 20">
                            <path d="M20.924 7.625a1.523 1.523 0 0 0-1.238-1.044l-5.051-.734-2.259-4.577a1.534 1.534 0 0 0-2.752 0L7.365 5.847l-5.051.734A1.535 1.535 0 0 0 1.463 9.2l3.656 3.563-.863 5.031a1.532 1.532 0 0 0 2.226 1.616L11 17.033l4.518 2.375a1.534 1.534 0 0 0 2.226-1.617l-.863-5.03L20.537 9.2a1.523 1.523 0 0 0 .387-1.575Z"/>
                        </svg>
                        <svg class="w-4 h-4 text-gray-300 dark:text-gray-500" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 22 20">
                            <path d="M20.924 7.625a1.523 1.523 0 0 0-1.238-1.044l-5.051-.734-2.259-4.577a1.534 1.534 0 0 0-2.752 0L7.365 5.847l-5.051.734A1.535 1.535 0 0 0 1.463 9.2l3.656 3.563-.863 5.031a1.532 1.532 0 0 0 2.226 1.616L11 17.033l4.518 2.375a1.534 1.534 0 0 0 2.226-1.617l-.863-5.03L20.537 9.2a1.523 1.523 0 0 0 .387-1.575Z"/>
                        </svg>
                        <h3 class="text-sm font-semibold text-gray-900 ms-2 dark:text-white">Thinking to buy another one!</h3>
                    </div>
                    <footer class="mb-5 text-sm text-gray-500 dark:text-gray-400"><p>Reviewed in the United Kingdom on <time datetime="2017-03-03 19:00">March 3, 2017</time></p></footer>
                    <p class="mb-2 text-gray-500 dark:text-gray-400">This is my third Invicta Pro Diver. They are just fantastic value for money. This one arrived yesterday and the first thing I did was set the time, popped on an identical strap from another Invicta and went in the shower with it to test the waterproofing.... No problems.</p>
                    <p class="mb-3 text-gray-500 dark:text-gray-400">It is obviously not the same build quality as those very expensive watches. But that is like comparing a Citroën to a Ferrari. This watch was well under £100! An absolute bargain.</p>
                    <a href="#" class="block mb-5 text-sm font-medium text-blue-600 hover:underline dark:text-blue-500">Read more</a>
                    <aside>
                        <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">19 people found this helpful</p>
                        <div class="flex items-center mt-3">
                            <a href="#" class="px-2 py-1.5 text-xs font-medium text-gray-900 focus:outline-none bg-white rounded-lg border border-gray-200 hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-4 focus:ring-gray-100 dark:focus:ring-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:text-white dark:hover:bg-gray-700">Helpful</a>
                            <a href="#" class="text-sm font-medium text-blue-600 border-gray-200 ps-4 hover:underline dark:text-blue-500 ms-4 border-s md:mb-0 dark:border-gray-600">Report abuse</a>
                        </div>
                    </aside>
                </article>

                </div>
                {{-- Card --}}

                 {{-- Card  --}}
            <div class="p-4 border rounded-lg">

                <article>
                    <div class="flex items-center mb-4">
                        <img class="w-10 h-10 rounded-full me-4" src="/docs/images/people/profile-picture-5.jpg" alt="">
                        <div class="font-medium dark:text-white">
                            <p>Jese Leos <time datetime="2014-08-16 19:00" class="block text-sm text-gray-500 dark:text-gray-400">Joined on August 2014</time></p>
                        </div>
                    </div>
                    <div class="flex items-center mb-1 space-x-1 rtl:space-x-reverse">
                        <svg class="w-4 h-4 text-yellow-300" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 22 20">
                            <path d="M20.924 7.625a1.523 1.523 0 0 0-1.238-1.044l-5.051-.734-2.259-4.577a1.534 1.534 0 0 0-2.752 0L7.365 5.847l-5.051.734A1.535 1.535 0 0 0 1.463 9.2l3.656 3.563-.863 5.031a1.532 1.532 0 0 0 2.226 1.616L11 17.033l4.518 2.375a1.534 1.534 0 0 0 2.226-1.617l-.863-5.03L20.537 9.2a1.523 1.523 0 0 0 .387-1.575Z"/>
                        </svg>
                        <svg class="w-4 h-4 text-yellow-300" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 22 20">
                            <path d="M20.924 7.625a1.523 1.523 0 0 0-1.238-1.044l-5.051-.734-2.259-4.577a1.534 1.534 0 0 0-2.752 0L7.365 5.847l-5.051.734A1.535 1.535 0 0 0 1.463 9.2l3.656 3.563-.863 5.031a1.532 1.532 0 0 0 2.226 1.616L11 17.033l4.518 2.375a1.534 1.534 0 0 0 2.226-1.617l-.863-5.03L20.537 9.2a1.523 1.523 0 0 0 .387-1.575Z"/>
                        </svg>
                        <svg class="w-4 h-4 text-yellow-300" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 22 20">
                            <path d="M20.924 7.625a1.523 1.523 0 0 0-1.238-1.044l-5.051-.734-2.259-4.577a1.534 1.534 0 0 0-2.752 0L7.365 5.847l-5.051.734A1.535 1.535 0 0 0 1.463 9.2l3.656 3.563-.863 5.031a1.532 1.532 0 0 0 2.226 1.616L11 17.033l4.518 2.375a1.534 1.534 0 0 0 2.226-1.617l-.863-5.03L20.537 9.2a1.523 1.523 0 0 0 .387-1.575Z"/>
                        </svg>
                        <svg class="w-4 h-4 text-yellow-300" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 22 20">
                            <path d="M20.924 7.625a1.523 1.523 0 0 0-1.238-1.044l-5.051-.734-2.259-4.577a1.534 1.534 0 0 0-2.752 0L7.365 5.847l-5.051.734A1.535 1.535 0 0 0 1.463 9.2l3.656 3.563-.863 5.031a1.532 1.532 0 0 0 2.226 1.616L11 17.033l4.518 2.375a1.534 1.534 0 0 0 2.226-1.617l-.863-5.03L20.537 9.2a1.523 1.523 0 0 0 .387-1.575Z"/>
                        </svg>
                        <svg class="w-4 h-4 text-gray-300 dark:text-gray-500" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 22 20">
                            <path d="M20.924 7.625a1.523 1.523 0 0 0-1.238-1.044l-5.051-.734-2.259-4.577a1.534 1.534 0 0 0-2.752 0L7.365 5.847l-5.051.734A1.535 1.535 0 0 0 1.463 9.2l3.656 3.563-.863 5.031a1.532 1.532 0 0 0 2.226 1.616L11 17.033l4.518 2.375a1.534 1.534 0 0 0 2.226-1.617l-.863-5.03L20.537 9.2a1.523 1.523 0 0 0 .387-1.575Z"/>
                        </svg>
                        <h3 class="text-sm font-semibold text-gray-900 ms-2 dark:text-white">Thinking to buy another one!</h3>
                    </div>
                    <footer class="mb-5 text-sm text-gray-500 dark:text-gray-400"><p>Reviewed in the United Kingdom on <time datetime="2017-03-03 19:00">March 3, 2017</time></p></footer>
                    <p class="mb-2 text-gray-500 dark:text-gray-400">This is my third Invicta Pro Diver. They are just fantastic value for money. This one arrived yesterday and the first thing I did was set the time, popped on an identical strap from another Invicta and went in the shower with it to test the waterproofing.... No problems.</p>
                    <p class="mb-3 text-gray-500 dark:text-gray-400">It is obviously not the same build quality as those very expensive watches. But that is like comparing a Citroën to a Ferrari. This watch was well under £100! An absolute bargain.</p>
                    <a href="#" class="block mb-5 text-sm font-medium text-blue-600 hover:underline dark:text-blue-500">Read more</a>
                    <aside>
                        <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">19 people found this helpful</p>
                        <div class="flex items-center mt-3">
                            <a href="#" class="px-2 py-1.5 text-xs font-medium text-gray-900 focus:outline-none bg-white rounded-lg border border-gray-200 hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-4 focus:ring-gray-100 dark:focus:ring-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:text-white dark:hover:bg-gray-700">Helpful</a>
                            <a href="#" class="text-sm font-medium text-blue-600 border-gray-200 ps-4 hover:underline dark:text-blue-500 ms-4 border-s md:mb-0 dark:border-gray-600">Report abuse</a>
                        </div>
                    </aside>
                </article>

                </div>
                {{-- Card --}}


        </div>



    </section>

    {{-- Review ZONE --}}


</div>
